...
void MainWindow::onStartClick()
{
    ...
    QString conflictMode = ui->conflictComboBox->currentText();
    bool overwrite = (conflictMode == "Перезаписать");
    // ==============================

    // ===== ПЕРЕДАЁМ ПАРАМЕТРЫ =====
    m_processor->setParams(sourcePath, destPath, mask, keyBytes, deleteOriginal, overwrite);
    // Запускаем поток
    m_thread->start();
